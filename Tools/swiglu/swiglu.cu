...
// Specialized SiLU for bfloat16 (only for Ampere and newer)
...
template<>
__device__ __forceinline__ void atomic_add_impl<c10::Half>(c10::Half* address, c10::Half val) {
    __half* half_addr = reinterpret_cast<__half*>(address);
    __half half_val = *reinterpret_cast<__half*>(&val);
    atomicAdd(half_addr, half_val);
}

// Specialization for c10::BFloat16 -> __nv_bfloat16 conversion (Ampere+)
#if defined(__CUDA_ARCH__) && __CUDA_ARCH__ >= 800
template<>
__device__ __forceinline__ void atomic_add_impl<c10::BFloat16>(c10::BFloat16* address, c10::BFloat16 val) {
    __nv_bfloat16* bf16_addr = reinterpret_cast<__nv_bfloat16*>(address);
    __nv_bfloat16 bf16_val = *reinterpret_cast<__nv_bfloat16*>(&val);
    atomicAdd(bf16_addr, bf16_val);
}
#endif
...
__global__ void swiglu_backward_kernel(
    ...
    if (idx < total_elements) {
        ...
        for (int h = 0; h < hidden_size; h++) {
            ...
            // gpuAtomicAdd handles Half/BFloat16 automatically on sm_75
            gpuAtomicAdd(&grad_x[x_offset + h], g_x);
            gpuAtomicAdd(&grad_w_gate[h * intermediate_size + inter_idx], grad_gate * x_val);
            gpuAtomicAdd(&grad_w_up[h * intermediate_size + inter_idx], grad_up * x_val);
        }
    }
}
